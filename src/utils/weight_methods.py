...
from src.utils.min_norm_solvers import MinNormSolver, gradient_normalizers
...
class STL(WeightMethod):
    ...
    def __init__(self, n_tasks, device: torch.device, main_task=0):
        super().__init__(n_tasks, device=device)
...
class Uncertainty(WeightMethod):
    ...
    SCHMce: https://github.com/yaringal/multi-task-learning-example/blob/master/multi-task-learning-example-pytorch.ipynb
    """
...
class FairGrad(WeightMethod):
    ...
    def backward(
        ...
        **kwargs,
        ...
        GTG, w = self.get_weighted_loss(losses, shared_parameters)
        loss = sum(w[i] * losses[i].item() for i in range(self.n_tasks))
        if self.max_norm > 0:
            torch.nn.utils.clip_grad_norm_(shared_parameters, self.max_norm)
        return loss, {"GTG": GTG, "weights": w}  # NOTE: to align with all other weight methods
