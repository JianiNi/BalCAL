...
# __all__ = ['PLP_Adam','Adam', 'adam']


class PLP_Adam(Optimizer):
    ...
    def show(self, closure=None):
        ...
        for i, group in enumerate(self.param_groups):
            ...
            for idx, p in enumerate(group['params']):
                name = None
                for n, param in self.state.items():
                    if param is p:
                        name = n
                        break
                print(f"  Index: {idx}, Name: {name if name else 'N/A'}, Shape: {p.shape}")


 
PLP_Adam.__doc__ = r"""Implements Adam algorithm.

    .. math::
       \begin{aligned}
            &\rule{110mm}{0.4pt}                                                                 \\
            &\textbf{input}      : \gamma \text{ (lr)}, \beta_1, \beta_2
                \text{ (betas)},\delta_0 \text{ (params)},f(\delta) \text{ (objective)}          \\
            ...
            &\hspace{10mm}g_t           \leftarrow   -\nabla_{\delta} f_t (\delta_{t-1})         \\
            &\hspace{5mm}\textbf{else}                                                           \\
            &\hspace{10mm}g_t           \leftarrow   \nabla_{\delta} f_t (\delta_{t-1})          \\
            &\hspace{5mm}\textbf{if} \: \lambda \neq 0                                           \\
            &\hspace{10mm} g_t \leftarrow g_t + \lambda  \delta_{t-1}                            \\
            ...
            &\hspace{10mm}\widehat{v_t}^{max} \leftarrow \mathrm{max}(\widehat{v_t}^{max},
                \widehat{v_t})                                                                   \\
            &\hspace{10mm}\delta_t \leftarrow \delta_{t-1} - \gamma \widehat{m_t}/
                \big(\sqrt{\widehat{v_t}^{max}} + \epsilon \big)                                 \\
            &\hspace{5mm}\textbf{else}                                                           \\
            &\hspace{10mm}\delta_t \leftarrow \delta_{t-1} - \gamma \widehat{m_t}/
                \big(\sqrt{\widehat{v_t}} + \epsilon \big)                                       \\
            &\rule{110mm}{0.4pt}                                                          \\[-1.ex]
            &\bf{return} \:  \delta_t                                                     \\[-1.ex]
            &\rule{110mm}{0.4pt}                                                          \\[-1.ex]
       \end{aligned}
